...
func diffWafIpSetDescriptors(oldD, newD []interface{}) []*waf.IPSetUpdate {
	...
	for _, od := range oldD {
		...
		if idx, contains := containsDescriptor(newD, descriptor); contains {
...
func containsDescriptor(l []interface{}, descriptor map[string]interface{}) (int, bool) {
	for i, d := range l {
		if reflect.DeepEqual(descriptor, d.(map[string]interface{})) {
			return i, true
		}
	}
